...
interface studentInfor {
	student: Leadership_Student_Add_Edit;
	classes: dataDeclaration_class[];
}
interface InforStudentProps {
	onInforStudent: () => void;
	onCancel: () => void;
	studentData?: Leadership_Student; // Optional student data for editing
	onUpdateStudent: (data: Leadership_Student) => void; // Callback to update student data
...
const InforStudent: FC<InforStudentProps> = ({
	onInforStudent,
	onCancel,
	studentData,
	onUpdateStudent,
	...
	const [studentInfor, setStudentInfor] = useState<studentInfor | null>(null);

	const [student, setStudent] = useState<Leadership_Student_Add_Edit>(
		studentData ? { ...studentData } : ({} as Leadership_Student_Add_Edit),
	);
	...
	const [dataClasses, setDataClasses] = useState<dataDeclaration_class[]>([]);
	...
	useEffect(() => {
		const fetchClass = async () => {
			...
			if (data.length > 0) {
				const studentClass = `${studentData?.gradeLevel}${studentData?.className}`;

				const filteredClasses = data.filter((d) => d.className === studentClass);
				console.log(filteredClasses);

				setStudentInfor({
					student, // Gán đúng kiểu dữ liệu của student
					classes: filteredClasses, // Gán danh sách lớp đã lọc
				});
			}
		};
		fetchClass();
	...
	// Sync student state with studentData when it changes
	useEffect(() => {
		if (studentData) {
			setStudent({ ...studentData }); // Spread to copy all properties except id if needed
			setAvatarPreview(
				studentData.avatar instanceof File
					? URL.createObjectURL(studentData.avatar)
					: studentData.avatar || null,
	...
	const handleCancelModal = () => setIsModal(false);
	...
	const handleSubmit = () => {
		if (onUpdateStudent && studentData?.id) {
			// Ensure the id is included in the submitted data
			const updatedStudent: Leadership_Student = {
				...student,
				id: studentData.id, // Add the id back from studentData
	...
	const toggleEdit = () => {
		if (isEditing && studentData) {
			setStudent({ ...studentData }); // Reset to original data if cancelling
			setAvatarPreview(
				studentData.avatar instanceof File
					? URL.createObjectURL(studentData.avatar)
					: studentData.avatar || null,
	...
	return (
		<div className=''>
			...
			{isActive === 'general' ? (
									...
									<div className='space-y-4'>
										...
										<div className='flex items-center gap-x-[55px]'>
											...
											<DatePicker
												...
												onChange={(date) => handleChange('birthDate', date ? date.toDate() : null)}
												className='h-10 w-[360px]'
												format='DD/MM/YYYY'
												disabled={!isEditing}
											/>
									...
									<div className='space-y-4'>
										...
										<div className='flex items-center gap-x-[55px]'>
											...
											<div className='flex gap-2'>
												<Select
													value={student.gradeLevel || undefined}
										...
										<div className='flex items-center gap-x-[55px]'>
											...
											<DatePicker
												...
												onChange={(date) =>
													handleChange('enrollmentDate', date ? date.toDate() : null)
												}
												className='h-10 w-[360px]'
												format='DD/MM/YYYY'
												disabled={!isEditing}
										...
										<div className='flex items-center gap-x-[55px]'>
											...
											<Select
												value={student.admissionType || undefined}
										...
										<div className='flex items-center gap-x-[55px]'>
											...
											<Select
												value={student.status || undefined}
				...
				<div>
					{studentInfor?.classes.map((classes: dataDeclaration_class) => (
						<div className='relative h-[258px] w-[1640px] overflow-hidden rounded-2xl border border-[#ff7506] bg-[#fff9f4]'>
							...
							<div className="font-['Source Sans Pro'] absolute left-[616px] top-[107px] text-base font-normal leading-tight text-[#373839] opacity-80">
								{classes.classQuantity} học viên
							</div>
