...
import moment from 'moment'; // Chỉ cần import moment, không cần Moment type riêng
import { examScheduleData } from '../../../types/leadership/instructor';
...
interface EditExamScheduleModalProps {
	visible: boolean;
	onCancel: () => void;
	onOk: (values: examScheduleData) => void;
	initialData: examScheduleData; // Dữ liệu lịch thi hiện tại để chỉnh sửa
}
...
const EditExamScheduleModal: React.FC<EditExamScheduleModalProps> = ({
	visible,
	onCancel,
	onOk,
	initialData,
	...
	const [customExamName, setCustomExamName] = useState<string>(''); // State cho Input tên kỳ thi
	const [duration, setDuration] = useState<string | undefined>(undefined);
	const [examDate, setExamDate] = useState<string>(''); // Dùng string giống AddExamScheduleModal
	...
	// Khởi tạo state với dữ liệu ban đầu khi modal mở
	useEffect(() => {
		if (visible && initialData) {
			...
			setCustomExamName(
				initialData.examName && initialData.examName.length > 0
					? initialData.examName[initialData.examName.length - 1]
					: '',
			); // Lấy tên tùy chỉnh cuối cùng nếu có
			setDuration(initialData.duration);
			setExamDate(initialData.examDate || ''); // Giữ nguyên string từ initialData
			setAssignmentType(initialData.assignmentType);
			setAllClassTeachers(initialData.allClassTeachers || []);
			setSpecificTeachers(initialData.specificTeachers || []);
			setNewAllTeacher('');
			setNewClass('');
	...
	const resetFields = () => {
		...
		setExamDate(''); // Reset về chuỗi rỗng giống AddExamScheduleModal
		setAssignmentType(undefined);
		setAllClassTeachers(['Nguyễn Văn D', 'Trần Thị D']); // Giống AddExamScheduleModal
		setSpecificTeachers([]);
		setNewAllTeacher('');
		setNewClass('');
		setNewSpecificTeacher('');
			...
			<div className='py-5'>
				<div className='space-y-10'>
					...
					<div className='flex h-5 items-center justify-between'>
						...
						<div className='h-10 w-[561px]'>
							<DatePicker
								value={examDate ? moment(examDate, 'DD/MM/YYYY') : null} // Chuyển string thành Moment khi hiển thị
								onChange={(date) => setExamDate(date ? date.format('DD/MM/YYYY') : '')} // Chuyển Moment về string khi thay đổi
								format='DD/MM/YYYY'
								className='h-10 w-full'
							/>
							{errors.examDate && (
